...
def encontrar_maior_numero(*numeros): # O asterisco antes de numeros indica que a função aceita um número variável de argumentos. Esses argumentos são empacotados em uma tupla chamada numeros.
    ...
    for n in numeros:                 # A função itera sobre cada número (n) na tupla numeros
        if maior_numero == 0:               
            break
        if n > maior_numero:          # Se o número atual (n) for maior que o valor armazenado em maior_numero, 
            maior_numero = n          #maior_numero é atualizado para o valor de n
